...
window.onload = function() {
  const container = document.querySelector('.button-containter');
  const buttons = container.querySelectorAll('.random-btn');
  ...
  buttons.forEach(btn => {
    ...
    const left = Math.random() * (containerWidth - btnWidth);
    const top = Math.random() * (containerHeight - btnHeight);
